...
if __name__ == "__main__":
    ...
    sr.bind(('',PORT_REPLICA))
    sr.listen(2)
    threads = []
    dialog = []

    while (True):
        src,address_r=sr.accept()
        print("Replica aceptada del socket {0[0]}:{0[1]}.".format(address_r))
        sc, address = s.accept()
        print("Conexión aceptada del socket {0[0]}:{0[1]}.".format(address))
        dialog.append(sc)
        t = threading.Thread(target=session, args=(dialog[-1],src))
        threads.append(t)
        t.start()
